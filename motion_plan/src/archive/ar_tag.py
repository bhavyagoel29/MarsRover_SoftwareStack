...
def get_warped_tags(image, tag_to_corners_map, ar_tag_contours):
    ...
    for key in tag_to_corners_map:
        if len(tag_to_corners_map[key]) != 0:
            warp = get_warped_image(
                np.float32(tag_to_corners_map[key]),
                dest_points,
                image,
                dest_img,
                [],
...
def get_warped_image(
    src_points, dest_points, src_img, dest_img, contour, is_dest_rect_plane
):
    H = get_homography(src_points, dest_points)
    warped_img = warp_img_perspective(
        dest_points, H, src_img, dest_img, contour, is_dest_rect_plane
    )
    return warped_img
...
def get_ar_tag_id(tag_image, corner_list, tag_str):
    ...
    id_number = [
        (is_cell_white(inner_corners_map[cell_key])) for cell_key in inner_corners_map
    ]
    print("id_number", id_number)
    re_orient_action_map = {
        "TR": [3, 0, 1, 2],
        "TL": [2, 3, 0, 1],
        "BL": [1, 2, 3, 0],
        "BR": [0, 1, 2, 3],
...
def is_cell_white(cell):
    threshold = 200
    cell_to_gray = (
        cv2.cvtColor((cell), cv2.COLOR_BGR2GRAY) if len(cell.shape) > 2 else cell
    )
    return 1 if (np.mean(cell_to_gray) >= threshold) else 0


def warp_img_perspective(
    dest_points, H, src_img, dest_img, dest_contour, is_dest_rect_plane
):
    # H - src to dest img
    ...
    for y_ind in range(int(row_min), int(row_max)):
        for x_ind in range(int(col_min), int(col_max)):
            ...
            if is_dest_rect_plane or (
                len(dest_contour) != 0
                and cv2.pointPolygonTest(dest_contour, (x_ind, y_ind), True) >= 0
            ):
                dest_pt = np.float32([x_ind, y_ind, 1]).T
                src_pt = np.dot(H_inv, dest_pt)
                src_pt = (src_pt / src_pt[2]).astype(int)
                if (src_pt[1] in range(0, src_img_dim[0])) and (
                    src_pt[0] in range(0, src_img_dim[1])
                ):
...
def get_homography(src_pts, dest_pts):
    ...
    for i in range(4):
        ...
        A_row_1 = np.asarray(
            [src_x, src_y, 1.0, 0, 0, 0, -src_x * dest_x, -src_y * dest_x]
        )
        A_row_2 = np.asarray(
            [0, 0, 0, src_x, src_y, 1.0, -src_x * dest_y, -src_y * dest_y]
        )
        A.append(A_row_1)
        A.append(A_row_2)
        b.append([dest_x])
    ...
    for elem in h:
        H.append(elem[0])
    H.append(1.0)  # we assumed last element of H to be one (1)
...
def project_cube(dest_points, contour, src_img):
    ...
    proj_corner_points, jacobian = cv2.projectPoints(
        axis_points, R_mat, t_vec, K, np.zeros((1, 4))
    )
...
def get_rotation_and_translation_matrix(K, H):
    ...
    lam = (
        np.linalg.norm(np.dot(K_inv, H[:, 0])) + np.linalg.norm(np.dot(K_inv, H[:, 1]))
    ) / 2
    lam = 1 / lam
...
def process_video(video_path, show_lena, show_cube):
    ...
    lena_img_corners = np.float32(
        [[0, 0], [lenaimg_w - 1, 0], [lenaimg_w - 1, lenaimg_h - 1], [0, lenaimg_h - 1]]
    )
        ...
        try:
            ...
            if not ret_val:
                ...
                if ar_tag_contours is not None:
                    ...
                    corners = [
                        get_corners_from_contours(contour)
                        for contour in ar_tag_contours
                    ]
                    ...
                    warped_tags = get_warped_tags(
                        gray, tag_to_corners_map, ar_tag_contours
                    )
                    ...
                    for ind, img in enumerate(warped_tags):
                        if img is not None:
                            ...
                            tag_num = tag_num + 1
                            tag_id, corner_list = get_ar_tag_id(
                                img, tag_to_corners_map[ind], tag_str
                            )
                            if tag_id is not None:
                                ...
                                cv2.putText(
                                    img_copy_for_id,
                                    "ID:" + str(tag_id),
                                    (corner_list[0][0] + 50, corner_list[0][1] + 10),
                                    font,
                                    0.5,
                                    (200, 0, 155),
                                    1,
                            ...
                            if show_lena:
                                img_copy_for_lena = get_warped_image(
                                    lena_img_corners,
                                    np.float32(corner_list),
                                    lena_img,
                                    img_copy_for_lena,
                                    ar_tag_contours[ind],
                                    False,
                            ...
                            if show_cube:
                                img_copy_for_cube = project_cube(
                                    np.float32(corner_list),
                                    ar_tag_contours[ind],
                                    img_copy_for_cube,
                                )
                    ...
                    con = cv2.drawContours(
                        id_display_img, ar_tag_contours, -1, (0, 255, 0), 2
                    )
...
if ar_tag_contours is not None:
    ...
    for ind, img in enumerate(warped_tags):
        if img is not None:
            ...
            if show_cube:
                img_copy_for_cube = project_cube(
                    np.float32(corner_list), ar_tag_contours[ind], img_copy_for_cube
                )
